...
class Verifyeblast extends CI_Controller {
	...
	function index()
	{
	   ...
	   if($this->form_validation->run() == FALSE)
	   {
			...
			if(($data['developer'] == '') || ($data['project'] == '')){
				...
				redirect('eblasts', 'refresh');
			}
			
			if($this->input->post("view_campaign") == "view"){
				$data['campaign_list'] = $this->input->post("campaign_list");
			}
	   ...
	   else
	   {
			...
			if($this->input->post("view_campaign") == "view"){
				$selected_devproj['campaign_list'] = $this->input->post("campaign_list");
			}		
